...
export const ProfilePhotoInput = ({ value, onChange, onNext, onBack }) => {
  ...
  return (
    <StepLayout
      subtitle={t('onboarding.subtitles.appearance')}
      title={t('onboarding.profile_photo.title')}
      description={t('onboarding.profile_photo.description')}
      onNext={onNext}
      onBack={onBack}
      nextDisabled={!value}
      scrollable={false}
      ...
      <div className="flex flex-col items-center justify-center h-full pb-4">
        <div className="relative group mx-auto w-full max-w-[220px]">
          <div className="w-full aspect-[4/5] rounded-[32px] border-2 border-dashed border-divider bg-background-warm/60 overflow-hidden flex items-center justify-center transition-all group-hover:border-primary/50 shadow-2xl">
            {preview ? (
              <img src={preview} alt={t('onboarding.profile_photo.preview_alt')} className="w-full h-full object-cover" />
            ) : (
              <div className="flex flex-col items-center gap-4 text-center p-6">
                ...
                <p className="text-ink/40 text-xs">
                  {t('onboarding.profile_photo.max_size')}
                </p>
              </div>
            )}
          </div>
